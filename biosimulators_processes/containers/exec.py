...
from biosimulators_processes.containers.io import write_dockerfile
...
def get_simulators(sims: List[str]):
    ...
    """
    with open('biosimulators_processes/_lock') as file:
...
def generate_dockerfile_contents(config: dict) -> str:
    ...
    base_path = 'biosimulators_processes/Dockerfile-base'
    ...
    with open(base_path, 'r') as fp:
        ...
        for simulator in config['simulators']:
            ...
            if name == 'copasi-basico':
                name = 'copasi'
            dockerfile_contents += f"COPY ./biosimulators_processes/{simulator['name']}_process.py /app/{simulator['name']}_process.py\n"
