...
class SimpleNeuronNetwork(Step):
    ...
    def __init__(self, config, core=CORE):
        ...
        model = nml_doc.add(model_name, model_id, **model_params_config)
        model.info(True)
        ...
        for i, synapse_config in enumerate(self.config['synapse_config'].items()):
            self.synapses[f'synapse_{i}'] = self._create_synapse(
                nml_doc=nml_doc, name=synapse_config[0], **synapse_config[1])

    def _create_synapse(self, nml_doc: object, name: str, syn_id: str, gbase: str, erev: str, tau_decay: str):
        return nml_doc.add(name, id=syn_id, gbase=gbase, erev=erev, tau_decay=tau_decay)
